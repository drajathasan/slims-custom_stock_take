...
if ($stk_q->num_rows) {
  ...
  if (isset($_POST['saveData']) AND empty($_POST['name'])) {
    ...
    if ($sql_op->insert('stock_take', $data)) {
      ...
      // collection type
      if (isset($_POST['collTypeID'])) {
        $ids = '(' . implode(',', array_map(fn($id) => (int)$id, $_POST['collTypeID'])) . ')';
        $criteria .= ' AND i.coll_type_id IN ' . $ids;
      }
      // location
      if ($_POST['location'] != '0') {
  ...
  $form->addCheckBox('collTypeID',  __('Collection Type'), $coll_type_options, []);
  // location
